...
variable "github_repository" {
  type    = string
  default = "CBarrosoBRRJ/PIPELINE-LOG-ORCAMENTO"
}
...
resource "google_project_service" "apis" {
  for_each = toset([
    ...
    "cloudscheduler.googleapis.com", "secretmanager.googleapis.com", "bigquery.googleapis.com",
...
resource "google_secret_manager_secret" "monday" {
  ...
  depends_on = [google_project_service.apis]
  lifecycle { prevent_destroy = true }
  # Add the actual version in the console: never put secret values in Terraform.
}
...
output "deploy_service_account" { value = google_service_account.deployer.email }
