...
-- Em outro ambiente, substitua orcamento pelo PG_SCHEMA correspondente.
...
JOIN orcamento.dim_status s ON s.status_id = i.current_status_id
...
JOIN orcamento.dim_item i USING (item_id, board_id)
...
FROM orcamento.fct_item_status_interval f
JOIN orcamento.dim_status s USING (status_id, board_id)
WHERE f.board_id = 18429499488 AND f.item_id = :item_id
...
FROM orcamento.fct_item_sla_summary r
JOIN orcamento.dim_item i USING (item_id, board_id)
...
WHERE f.board_id = 18429499488
  ...
  AND NOT EXISTS (
      SELECT 1 FROM orcamento.data_quality_issue q
      WHERE q.board_id = f.board_id AND q.item_id = f.item_id
        AND q.code IN ('cadeia_status_inconsistente', 'snapshot_status_divergente')
  )
...
FROM orcamento.dim_item i
JOIN orcamento.dim_status s ON s.status_id = i.current_status_id
JOIN orcamento.fct_item_sla_summary r ON r.item_id = i.item_id AND r.board_id = i.board_id
LEFT JOIN orcamento.fct_item_status_interval f
...
FROM orcamento.etl_watermark WHERE board_id = 18429499488;
